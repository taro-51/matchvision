...
function ClubPulse() {
  ...
  return <div className="community-page"><HubHeading eyebrow="WHOLE-CLUB INTELLIGENCE" title="Club Pulse" copy="A live view of participation, people, operations and community health across Springvale City."><span className="pulse-updated">Updated from latest match analysis</span></HubHeading><section className="pulse-grid">{pulseMetrics.map(([label,value,trend,direction,icon,suffix]) => <article key={label}><div><i>{icon}</i><span className={direction}>{direction === "up" ? "↗" : "↘"} {trend}</span></div><AnimatedNumber value={label === "Community Engagement" ? Number.parseInt(intelligence.club.engagement) : label === "Equipment Status" ? Number.parseInt(intelligence.club.equipmentReadiness) : value} suffix={suffix} /><p>{label}</p><small>Compared with last month</small></article>)}</section><section className="pulse-ai-card"><div className="pulse-ai-icon">AI</div><div><span>MATCHVISION CLUB INSIGHT · {intelligence.club.analysedMatches} MATCHES</span><h2>Player development is rising while operational readiness remains healthy.</h2><ul><li>Latest team priority: {intelligence.findings.priority}.</li><li>Player development trend is {intelligence.club.developmentTrend}.</li><li>Community engagement is {intelligence.club.engagement} and trending upwards.</li><li>Equipment readiness is {intelligence.club.equipmentReadiness} for upcoming sessions.</li></ul></div><strong>LIVE<br />SUMMARY</strong></section><section className="pulse-trends"><article><span>ATTENDANCE TREND</span><div className="pulse-bars">{[66,72,69,81,84,92].map((height,index) => <i key={index} style={{ height: `${height}%` }}><small>W{index+1}</small></i>)}</div></article><article><span>COMMUNITY ENGAGEMENT</span><div className="pulse-ring"><strong>{intelligence.club.engagement}</strong><small>Healthy and rising</small></div></article></section></div>;
}
