...
export default function AIStudio({ isOpen = true, embedded = false, role, user, onClose = () => {}, onNavigate }) {
  ...
  return <div className={embedded ? "ai-studio-destination" : "ai-studio-backdrop"} role={embedded ? undefined : "dialog"} aria-modal={embedded ? undefined : "true"} aria-label="MatchVision AI Studio" onClick={() => !embedded && !["processing", "briefing"].includes(status) && onClose()}>
    <section className="ai-studio" onClick={(event) => event.stopPropagation()}>
      ...
      {status === "complete" && <><div className="ai-studio-complete"><div className="ai-studio-complete-video ai-studio-video">{videoUrl ? <video src={videoUrl} autoPlay muted loop playsInline controls /> : <div className="ai-studio-demo-art"><img src={springvaleLogo} alt="Springvale City Soccer Club" /><span>ANALYSED MATCH</span><strong>Springvale City vs Oakleigh United</strong><small>Video and report remain connected</small></div>}</div><div className="ai-studio-complete-mark">OK</div><span>ANALYSIS COMPLETE</span><h1>Your connected Match Intelligence report is ready below.</h1><p>Match Library, development, recognition, highlights, Football Intelligence and the dashboard now share this single result.</p><div className="ai-studio-result-grid"><button type="button" onClick={() => openResult("matches")}><strong>View Match Library</strong><span>New match record created</span></button><button type="button" onClick={() => openResult("highlights")}><strong>View Highlights</strong><span>11 key moments detected</span></button><button type="button" onClick={() => openResult("dashboard")}><strong>Return to Dashboard</strong><span>Latest AI update connected</span></button></div><button type="button" onClick={() => { setFile(null); setVideoUrl(""); setStatus("ready"); }}>Analyse Another Match</button></div><MatchIntelligenceReport role={role} user={user} onNavigate={openResult} /></>}
    </section>
  </div>;
}
