...
import { archiveMatchWorkspace } from "../lib/matchArchive";
...
export default function AIStudio({ isOpen = true, embedded = false, role, user, onClose = () => {}, onNavigate }) {
  ...
  function completeAnalysis() {
    ...
    const generatedMatch = { id: `studio-${Date.now()}`, title: "Springvale 3-2 Oakleigh", date: "Analysed just now", status: "AI complete", visibility: ["parent", "player"].includes(role) ? "Approved personal replay" : "Club coaching staff", highlights: 11, approved: true, playerIds: [playerId], sourceFile: connected.sourceFile, completedAt };
    generatedMatch.workspaceId = generatedMatch.id;
    try {
      archiveMatchWorkspace(generatedMatch, connected);
